...
def check_mkcert_certificates():
    """Check if mkcert-generated certificates exist and return the most recent pair."""
    import glob
    # mkcert creates files like: anton.local+2.pem, anton.local+3.pem, etc.
...
def check_mkcert_installed():
    ...
    # Check for mkcert.exe in current directory first (local copy)
    local_mkcert = os.path.join(os.getcwd(), 'mkcert.exe')
...
def generate_self_signed_cert():
    ...
    # Check if the configured certificate files exist (might be from previous mkcert run)
